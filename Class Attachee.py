class Attachee: #Class representing an intern/attachee
    
    def __init__(self, name, division): #Initialize an attachee with a name and division.
        self.name = name
    ...
    def assign_task(self, task): #assigning tasks
    ...
    def add_feedback(self, task, feedback): #Add feedback for a specific task.
    ...
    def add_score(self, task, score): #Add a score for a specific task.
        if task in self.tasks:
            if 0 <= score <= 10:
    ...
    def _update_average_score(self): #Update the average score based on all task scores.
        if self.scores:
            self.average_score = sum(self.scores.values()) / len(self.scores)
        else:
            self.average_score = 0
    
    def get_performance_summary(self): #Get a summary of the attachee's performance.
...
class TechHub: #Class representing the Tech Innovation Hub with all divisions.
    
    def __init__(self): #Initialize the Tech Hub with empty divisions.
    ...
    def add_attachee(self, name, division): #Add a new attachee to the specified division.
    ...
    def get_attachee(self, name): #Get an attachee by name.
        for attachee in self.attachees:
            if attachee.name.lower() == name.lower():
                return attachee
        return None
    
    def get_attachees_by_division(self, division): #Get all attachees in a specific division.
        if division in self.divisions:
            return [a for a in self.attachees if a.division == division]
        else:
            return []
    
    def assign_task_by_division(self, division, task): #Assign a task to all attachees in a division.
        if division in self.divisions:
    ...
    def display_division_performance(self, division): #Display performance of all attachees in a division.
        if division in self.divisions:
            attachees = self.get_attachees_by_division(division)
            if attachees:
    ...
    def display_all_attachees(self): #Display all attachees grouped by division.
